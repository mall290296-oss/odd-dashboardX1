      ...
      <nav className="border-b border-slate-200 px-4 md:px-8 py-4 sticky top-0 bg-white/90 backdrop-blur-md z-50 shadow-sm print:hidden">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveTab("Accueil")}>
            ...
            <span className="text-xl md:text-2xl font-black tracking-tighter text-blue-600">ODD-X</span>
          </div>
          <div className="flex gap-3 md:gap-6 text-[10px] md:text-xs font-bold uppercase tracking-widest overflow-x-auto pb-2 md:pb-0 w-full md:w-auto no-scrollbar">
            {["Accueil", "À Propos", "Diagnostic", "Résultats", "Priorités", "Partenaires", "Citoyens", "Contact"].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`${activeTab === tab ? "text-blue-600 border-b-2 border-blue-600" : "text-slate-500 hover:text-blue-500"} pb-1 transition-all whitespace-nowrap`}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-12">
        {activeTab === "Accueil" && (
          <div className="text-center py-10 md:py-20 space-y-6 md:space-y-8 animate-in fade-in duration-1000">
            ...
            <div className="pt-6">
              <button onClick={() => setActiveTab("Diagnostic")} className="bg-blue-600 hover:bg-blue-700 text-white px-8 md:px-12 py-4 md:py-5 rounded-full font-black text-base md:text-lg transition-all hover:scale-105 shadow-xl shadow-blue-200 uppercase">DÉMARRER LE DIAGNOSTIC</button>
            </div>
          </div>
        )}
          ...
          <div className="max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in fade-in">
             <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
              <div className="flex flex-col sm:flex-row items-center md:items-end gap-4 w-full md:w-auto">
                ...
                {muralInfo["Nom de la commune"] && <button onClick={handleDeleteCurrentProfile} className="bg-red-50 text-red-600 border border-red-100 px-4 py-2 rounded-lg text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all w-full md:w-auto">Supprimer</button>}
              </div>
              <button onClick={handleNewProfile} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-black text-xs uppercase hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 w-full md:w-auto">➕ Nouvelle Mairie</button>
            </div>
            {Object.entries(identityFields).map(([category, fields]) => (
              <div key={category} className="bg-white p-6 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 shadow-sm">
            ...
            <div className="text-center pt-8">
              <button disabled={!isFullyIdentified} onClick={() => setActiveTab("Questionnaire")} className={`w-full md:w-auto px-12 py-5 rounded-2xl font-black uppercase transition-all shadow-2xl ${isFullyIdentified ? "bg-blue-600 text-white scale-105 shadow-blue-200" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>{isFullyIdentified ? "Passer au Questionnaire" : "Remplir tous les champs"}</button>
            </div>
          </div>
        )}
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                  {activeDiagnosticSection && (
                    <button 
                      ...
                      className="bg-blue-600 text-white px-3 py-2 rounded-xl text-[10px] md:text-xs font-black uppercase hover:bg-blue-700 transition-all shadow-md shrink-0"
                    >
                      ← Retour
                    </button>
                  )}
                  <p className="text-xs md:text-sm font-black uppercase text-slate-800 italic truncate">
                    Mairie : <span className="text-blue-600">{muralInfo["Nom de la commune"]}</span>
                  </p>
                </div>
                <button onClick={() => setActiveTab("Diagnostic")} className="w-full md:w-auto bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded-full text-[10px] font-black uppercase text-slate-600">Modifier Infos</button>
              </div>
              ...
              {!activeDiagnosticSection ? (
                  ...
                  {groupedQuestions.map((group) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative min-h-[300px] md:aspect-[3/4] ${theme.bg} rounded-[30px] shadow-2xl p-8 md:p-10 flex flex-col justify-center items-center text-center group hover:scale-[1.03] ${theme.hover} transition-all duration-300 overflow-hidden border-4 border-transparent hover:border-white/20`}
                      >
                        <div 
                          className={`absolute bottom-0 left-0 w-full ${theme.progress} transition-all duration-1000`} 
                          style={{ height: `${progress.percent}%` }}
                        ></div>
                        <h3 className="relative z-10 text-white text-xl md:text-2xl font-black uppercase tracking-tighter leading-tight mb-4">
                        ...
                        <div className="relative z-10 mt-8 md:mt-10 bg-white text-slate-900 px-8 py-3 rounded-full text-[10px] md:text-xs font-black uppercase tracking-widest group-hover:scale-110 transition-transform shadow-lg">
                ...
                <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right-10">
                  {groupedQuestions
                    ...
                    .map((group) => (
                      <div key={group.id} className="space-y-6 md:space-y-8">
                        <div className="flex flex-col md:flex-row justify-between items-center md:items-end border-b-4 border-blue-600 pb-4 gap-2">
                          <h3 className="text-xl md:text-4xl font-black text-slate-900 italic uppercase leading-tight text-center md:text-left">
                            {group.title}
                          </h3>
                          <span className="text-blue-600 font-black tracking-widest uppercase text-[10px] md:text-sm">
                            {getGroupProgress(group.questions).percent}% Complété
                          </span>
                        </div>
                        {group.questions.map((q) => (
                          <div key={q.id} className="bg-white p-6 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 shadow-sm">
                            <div className="flex flex-wrap gap-2 mb-4">
                              {q.odds.map(o => <span key={o} className="text-[8px] md:text-[9px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded font-black uppercase tracking-tighter">ODD {o}</span>)}
                            </div>
                            <div className="text-lg md:text-xl mb-6 text-slate-800 leading-snug">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 md:gap-6 pt-10 pb-20">
                    <button onClick={() => { setActiveDiagnosticSection(null); window.scrollTo(0,0); }} className="w-full md:flex-1 bg-slate-800 text-white p-5 rounded-2xl font-black uppercase hover:bg-slate-900 transition-all shadow-xl">← Retour au menu</button>
                    <button onClick={() => { window.scrollTo(0,0); setActiveTab("Résultats"); }} className="w-full md:flex-1 bg-blue-600 text-white p-5 rounded-2xl font-black uppercase shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all">Voir les résultats</button>
                  </div>
                </div>
              )}
           </div>
        ...
        {activeTab === "Résultats" && (
          <div className="space-y-8 md:space-y-12 animate-in slide-in-from-bottom-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1 bg-blue-600 p-10 md:p-16 rounded-[40px] md:rounded-[50px] flex flex-col items-center justify-center border border-white/20 shadow-2xl text-center text-white relative overflow-hidden">
                <div className="relative z-10">
                  <div className="text-7xl md:text-9xl font-black leading-none">{globalScore}</div>
                  <span className="text-xl md:text-2xl font-bold uppercase mt-4 block">Score / 5.0</span>
                </div>
              </div>
              <div className="lg:col-span-2 bg-white rounded-[40px] md:rounded-[50px] p-4 md:p-8 border border-slate-200 shadow-sm flex items-center justify-center overflow-x-auto">
                <div className="min-w-[300px] w-full">
                  <ReactECharts option={chartOption} style={{ height: "400px md:600px", width: "100%" }} />
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === "Priorités" && (
          <div className="space-y-8 animate-in fade-in">
            <div className="space-y-4">
              ...
              <p className="text-slate-500 text-sm md:text-lg max-w-4xl leading-relaxed italic border-l-4 border-slate-200 pl-4 md:pl-6">
                "Nous ne vous proposons ici que des recommandations générales. Si vous avez besoin d'une approche spécifique, veuillez contacter un spécialiste ou consulter la liste des institutions publiques."
              </p>
            </div>
            <div className="grid gap-4 md:gap-6">
              {lowPerformingODDs.map(item => {
                ...
                return (
                  <div key={item.odd} className={`bg-white p-6 md:p-8 rounded-[30px] border-l-[12px] md:border-l-[20px] ${visuals.twBorder} flex flex-col md:flex-row justify-between items-center gap-4 shadow-md border border-slate-200`}>
                    <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 text-center md:text-left">
                      <img src={oddIcons[item.odd]} alt={item.odd} className="w-16 h-16 md:w-20 md:h-20 rounded-xl" />
                      <div>
                        <div className={`text-3xl md:text-5xl font-black ${visuals.twText} italic uppercase leading-none mb-2`}>{item.odd}</div>
                        <p className="text-sm md:text-lg font-bold text-slate-700">{oddDescriptions[item.odd]}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-3xl md:text-5xl font-black text-slate-900">{item.value} <span className="text-xs md:text-sm text-slate-400">/ 5</span></p>
                    </div>
                  </div>
                );
              })}
        ...
        {activeTab === "À Propos" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center py-6 md:py-12 animate-in">
            <div className="space-y-6 md:space-y-8 text-center md:text-left">
              <h2 className="text-4xl md:text-6xl font-black italic underline decoration-blue-500 decoration-8 underline-offset-8 uppercase leading-tight text-slate-900 px-4">Notre Mission</h2>
              <p className="text-lg md:text-xl text-slate-600 leading-relaxed font-light px-4">ODD-X transforme les données communales en leviers d'action. En alignant votre stratégie sur les Objectifs de Développement Durable, nous créons ensemble des territoires résilients.</p>
            </div>
            <div className="rounded-[40px] overflow-hidden border border-slate-200 shadow-2xl mx-4">
        ...
        {activeTab === "Partenaires" && (
          <div className="space-y-8 md:space-y-12 animate-in fade-in">
            <div className="space-y-4">
              <h2 className="text-3xl md:text-5xl font-black italic uppercase underline decoration-blue-500 text-slate-900 leading-tight px-4">Institutions</h2>
              <p className="text-slate-500 text-sm md:text-lg max-w-3xl leading-relaxed px-4">Ces organismes publics et réseaux d'experts peuvent vous accompagner dans votre transition.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              {[
                ...
                <div key={i} className="bg-white p-6 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 shadow-sm hover:shadow-xl transition-all">
                  ...
                  <h3 className="text-lg md:text-xl font-black text-slate-900 uppercase mb-2">{inst.full}</h3>
                  <p className="text-slate-500 text-xs md:text-sm leading-relaxed mb-6">{inst.desc}</p>
                  <a href={inst.link} target="_blank" rel="noreferrer" className="text-blue-600 font-black text-[10px] md:text-xs uppercase tracking-widest border-b-2 border-blue-100 hover:border-blue-600 transition-all">Consulter</a>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 animate-in fade-in">
             <div className="lg:col-span-1 bg-white p-6 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 h-fit shadow-lg sticky top-24">
                ...
                <form onSubmit={handleAddIdea} className="space-y-4">
                  ...
                  <textarea name="ideaText" placeholder="Votre proposition..." rows="6" className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-100 text-sm" required></textarea>
                  <button type="submit" className="w-full bg-blue-600 text-white p-4 rounded-xl font-black uppercase shadow-lg hover:bg-blue-700 transition-all">Publier</button>
                </form>
             </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {citizenIdeas.map((idea, idx) => (
                    <div key={idx} className="bg-white p-5 md:p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between group relative transition-all hover:border-blue-200">
                      <button onClick={() => handleDeleteIdea(idx)} className="absolute -top-2 -right-2 bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg z-10">✕</button>
                      <div className="flex gap-4 mb-4">
                         <img src={oddIcons[idea.odd]} alt="" className="w-8 h-8 md:w-10 md:h-10 rounded-md shrink-0" />
                      ...
                      <div className="flex justify-between items-center mt-auto border-t border-slate-50 pt-4">
                        ...
                        <span className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase">Le {idea.date}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "Contact" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 py-12 items-center animate-in fade-in">
            <div className="space-y-6 md:space-y-8 text-slate-600 text-lg md:text-xl font-light text-center md:text-left px-4">
            ...
            <form action="https://formspree.io/f/xwvnldkr" method="POST" className="bg-white p-6 md:p-12 rounded-[40px] md:rounded-[50px] border border-slate-200 space-y-4 shadow-xl mx-4">
              <input type="text" name="name" placeholder="NOM" className="w-full bg-slate-50 border border-slate-100 p-4 md:p-6 rounded-2xl font-bold text-sm" required />
              <input type="email" name="email" placeholder="EMAIL" className="w-full bg-slate-50 border border-slate-100 p-4 md:p-6 rounded-2xl font-bold text-sm" required />
              <textarea name="message" placeholder="MESSAGE..." rows="5" className="w-full bg-slate-50 border border-slate-100 p-4 md:p-6 rounded-2xl font-bold text-sm" required></textarea>
              <button type="submit" className="w-full bg-blue-600 text-white p-4 md:p-6 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-700 transition-all">Envoyer</button>
            </form>
          </div>
        )}
      </div>
